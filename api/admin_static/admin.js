...
async function refreshKeyStatus() {
  try {
    const status = await api("/admin/api/keys/status");
    byId("key-stat-total").textContent = status.total_keys;
    byId("key-stat-healthy").textContent = status.healthy_keys;
    byId("key-stat-exhausted").textContent = status.exhausted_keys;
    byId("key-stat-current").textContent = status.current_key_index;
  } catch (error) {
