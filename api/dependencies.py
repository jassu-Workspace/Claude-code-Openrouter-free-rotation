...
def require_api_key(
    ...
    # Strip sk-ant- prefix if the configured token doesn't use it, to satisfy Claude Code CLI formatting rules
    if token.startswith("sk-ant-") and not anthropic_auth_token.startswith("sk-ant-"):
        token = token[7:]
