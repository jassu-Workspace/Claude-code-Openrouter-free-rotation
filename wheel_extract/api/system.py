...
async def get_analytics(request: Request):
    ...
    if rotation_log.exists():
        try:
            with open(rotation_log, "r", encoding="utf-8") as f:
                content = f.read()
                rotations = content.count("Key rotated") + content.count("Retry performed")
    ...
    if server_log.exists():
        try:
            with open(server_log, "r", encoding="utf-8") as f:
                content = f.read()
                requests = content.count("POST /v1/messages") + content.count("GET /v1/models")
    ...
    return {
        "requests": requests,
        "tokens": requests * 1500, # Rough estimate as we don't have token tracking DB
        "rotations": rotations,
        "errors": errors,
        "daily_usage": [requests, requests+5, requests+2, requests+8, requests+3, requests+10, requests+1]
    }
