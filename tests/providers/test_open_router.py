...
def open_router_provider(open_router_config):
    with patch("config.paths.openrouter_keys_path") as mock_path:
...
def test_init(open_router_config):
    ...
    with patch("httpx.AsyncClient") as mock_client, patch("config.paths.openrouter_keys_path") as mock_path:
        mock_path.return_value.is_file.return_value = False
